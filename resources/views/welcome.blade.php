...
<head>
    ...
    <!-- Favicon (menggantikan logo PPID yang terbawa) -->
    <link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='20' fill='%23080808'/%3E%3Crect width='96' height='96' x='2' y='2' rx='18' fill='none' stroke='%2338bdf8' stroke-width='4'/%3E%3Ctext x='50%25' y='50%25' font-family='Arial, sans-serif' font-size='48' font-weight='bold' fill='%23ffffff' text-anchor='middle' dominant-baseline='central'%3ESS%3C/text%3E%3C/svg%3E">
    ...
    <style>
        ...
        .hero-showcase {
            ...
            inset: 0;
            overflow: hidden;
            /* Vertical Parallax 3D Perspective - Left side text focus */
            transform: perspective(1000px) rotateX(15deg) rotateY(-5deg) scale(1.2);
            transform-origin: 75% center;
            opacity: 1; /* Opacity full, gradient will hide left side */
        }
        .hero-showcase::after {
            content: '';
            position: absolute;
            ...
            z-index: 2;
            background:
                linear-gradient(to right, #050505 0%, rgba(5,5,5,0.95) 40%, rgba(5,5,5,0.4) 60%, transparent 100%),
                radial-gradient(ellipse at 80% 50%, rgba(5,5,5,0) 0%, rgba(5,5,5,0.85) 60%, #050505 100%),
                linear-gradient(to top, #050505 0%, transparent 35%),
                linear-gradient(to bottom, #050505 0%, transparent 25%);
        }
        .showcase-rows {
            display: flex;
            flex-direction: row;
        ...
        .showcase-row:nth-child(odd) { animation: scrollUp 50s linear infinite; }
        .showcase-row:nth-child(even) { animation: scrollDown 50s linear infinite; }
        
        .showcase-row:nth-child(1) { animation-duration: 48s; margin-top: -150px; }
        .showcase-row:nth-child(2) { animation-duration: 65s; margin-top: -300px; }
        .showcase-row:nth-child(3) { animation-duration: 42s; margin-top: -100px; }
        .showcase-row:nth-child(4) { animation-duration: 58s; margin-top: -250px; }
        .showcase-row:nth-child(5) { animation-duration: 70s; margin-top: -400px; }
        ...
        /* ===== SECTION BACKDROPS (Ashley-inspired) ===== */
        .section-backdrop {
            position: absolute;
            inset: 0;
            overflow: hidden;
        ...
        /* Persistent side vignette glows */
        .edge-glow-left,
        .edge-glow-right {
            position: absolute;
            top: -20%;
        ...
        .wireframe-shape svg * {
            fill: none;
            stroke: rgba(165,140,255,0.4);
            stroke-width: 0.5;
        }

        /* Rotation keyframes for wireframes */
        ...
        .light-streak {
            position: absolute;
            height: 1px;
            background: linear-gradient(90deg, transparent 0%, rgba(165,140,255,0.08) 30%, rgba(165,140,255,0.12) 50%, rgba(165,140,255,0.08) 70%, transparent 100%);
        }
        ...
        .project-card:hover {
            ...
            box-shadow: 0 16px 48px -12px rgba(255,255,255,0.04);
        }
        ...
        .pl-text-anim {
            ...
            animation: phase1 2.2s cubic-bezier(0.2, 0.8, 0.2, 1) forwards;
        }
        @keyframes phase1 {
        ...
        .pl-reveal-anim {
            animation: phase2-show 0.1s linear 2.1s forwards;
        }
        ...
        .pl-reveal-block {
            ...
            transform: scaleX(0);
            animation: block-reveal 1.2s cubic-bezier(0.77, 0, 0.175, 1) 2.1s forwards;
        }
        .pl-reveal-text {
            color: transparent;
            animation: text-reveal 0.1s linear 2.7s forwards;
        }
...
<body class="antialiased overflow-hidden" id="body-wrapper">

    {{-- ============================================ --}}
    {{-- ASHLEY-STYLE PRELOADER --}}
    {{-- ============================================ --}}
    <script>
        // Deteksi apakah user melakukan refresh/reload halaman secara manual (Ctrl+R / F5)
        ...
        if (isReload) {
            // Hapus memori session agar preloader main ulang
            sessionStorage.removeItem('portfolioLoaded');
            sessionStorage.removeItem('portfolioScrollPos'); // Bersihkan sisa scroll memori
        }

        // Mencegah *flicker* layar hitam saat bernavigasi kembali dari halaman demo
        if (sessionStorage.getItem('portfolioLoaded')) {
            document.documentElement.classList.add('hide-preloader');
        }
    </script>
    <style>
        html.hide-preloader #preloader { display: none !important; }
        html.hide-preloader body#body-wrapper { overflow: visible !important; overflow-x: hidden !important; }
    </style>
    ...
    {{-- ============================================ --}}
    <div id="overlay-menu" class="overlay-menu fixed inset-0 z-[60] bg-[#0a0a0a] flex flex-col justify-between">
        ...
        <div class="relative z-10 flex-1 flex items-center px-8 sm:px-12 lg:px-20">
            <nav class="space-y-3">
                @foreach ([
                    ...
                    <a href="{{ $href }}" class="overlay-link flex items-baseline gap-5 group py-1">
        ...
        <div class="relative z-10 flex items-center justify-between px-8 sm:px-12 lg:px-20 py-6 text-xs text-white/20">
            <span>&copy; {{ date('Y') }} Sarham San</span>
    ...
    {{-- NAVBAR --}}
    {{-- ============================================ --}}
    <nav id="navbar" class="fixed top-0 left-0 right-0 z-[70] px-8 sm:px-12 lg:px-16 transition-all duration-300">
        <div class="flex items-center justify-between h-20">
            ...
            {{-- Mobile hamburger --}}
            <button id="menu-toggle" class="relative z-[70] w-10 h-10 flex md:hidden flex-col items-center justify-center gap-[6px]" aria-label="Menu">
                <span id="bar1" class="block w-6 h-[1.5px] bg-white transition-all duration-300 origin-center"></span>
                <span id="bar2" class="block w-6 h-[1.5px] bg-white transition-all duration-300 origin-center"></span>
            </button>
        </div>
    </nav>
    ...
    <section id="hero" class="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        {{-- Background vertical masonry showcase --}}
        <div class="hero-showcase">
            <div class="showcase-rows">
                @php
                    ...
                    // Acak komposisi kolom agar parallax terlihat asimetris & organik
                    $cols = [
                        $templateUrls,
                        array_reverse($templateUrls),
                        array_merge(array_slice($templateUrls, 5), array_slice($templateUrls, 0, 5)),
                        array_merge(array_slice($templateUrls, 3), array_slice($templateUrls, 0, 3), array_slice($templateUrls, 6), array_slice($templateUrls, 0, 1)),
                        array_merge(array_slice($templateUrls, 7), array_slice($templateUrls, 0, 7)),
                    ];
                @endphp

                @foreach($cols as $colUrls)
                    <div class="showcase-row">
                        {{-- Mengulang 2x agar endless loop berjalan sempurna saat translateY mencapai -50% --}}
                        @for ($r = 0; $r < 2; $r++)
                            @foreach ($colUrls as $url)
                                <div class="web-card">
                                    ...
                                    <div class="web-card-body">
                                        <iframe src="{{ $url }}" loading="lazy" sandbox="allow-same-origin" tabindex="-1"></iframe>
                                    </div>
                                </div>
                            @endforeach
                        @endfor
        ...
        {{-- Content --}}
        <div id="parallax-text" class="relative z-10 w-full px-8 sm:px-12 lg:px-16 max-w-7xl mx-auto flex flex-col items-start text-left mt-10 lg:mt-20">
            ...
            <h1 class="reveal text-5xl sm:text-7xl lg:text-[6rem] font-extrabold tracking-[-0.02em] leading-[1.05] mb-8 text-white">
                ...
                Merancang <br>
                <span id="role-text" class="text-transparent bg-clip-text bg-gradient-to-r from-white via-white/90 to-white/40 inline-block font-black italic mt-1 pr-4">Web Developer</span>
            </h1>
            ...
            <div class="marquee-track flex whitespace-nowrap">
                @for ($i = 0; $i < 3; $i++)
                    <div class="flex items-center gap-12 mx-8 shrink-0">
                        @foreach (['Laravel', 'Tailwind CSS', 'PHP', 'JavaScript', 'MySQL', 'Figma', 'Git', 'Vite'] as $tech)
    ...
    <section id="skills" class="py-28 lg:py-36 border-t border-white/[0.05] relative">
        ...
        <div class="w-full px-8 sm:px-12 lg:px-16 xl:px-24 relative z-10">
            ...
            <div class="mt-12 flex justify-center w-full z-20 relative">
                <button id="btn-toggle-demos" class="group relative px-6 py-3 rounded-full bg-white/[0.02] border border-white/[0.08] hover:bg-white/[0.05] hover:border-white/20 transition-all duration-300 flex items-center gap-3">
                    <span id="btn-toggle-text" class="text-[13px] font-semibold text-white/70 tracking-wide uppercase group-hover:text-white transition-colors">Lihat Semua Demo (12)</span>
                    <svg id="btn-toggle-icon" class="w-4 h-4 text-white/40 group-hover:text-white transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
            </div>

            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    const toggleBtn = document.getElementById('btn-toggle-demos');
                    const toggleText = document.getElementById('btn-toggle-text');
                    const toggleIcon = document.getElementById('btn-toggle-icon');
                    const hiddenDemos = document.querySelectorAll('.hidden-demo');
                    let isExpanded = false;

                    toggleBtn.addEventListener('click', () => {
                        isExpanded = !isExpanded;
                        hiddenDemos.forEach(demo => {
                            if(isExpanded) {
                                demo.classList.remove('hidden');
                                // slight delay for animation
                                setTimeout(() => demo.classList.add('opacity-100'), 50);
                            } else {
                        ...
                        if(isExpanded) {
                            toggleText.textContent = 'Sembunyikan Sebagian';
                            toggleIcon.style.transform = 'rotate(180deg)';
                        } else {
                            toggleText.textContent = 'Lihat Semua Demo (12)';
            ...
            <div class="grid lg:grid-cols-2 gap-16 lg:gap-20">
                ...
                <div class="reveal">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
                        @php
                            $techs = [
                                'Laravel', 'PHP', 'Python', 'Java',
                                'Svelte', 'Tailwind', 'JavaScript', 'HTML/CSS',
                                'MySQL', 'Figma', 'Git'
                            ];
                        @endphp
                        @foreach ($techs as $index => $tech)
                            <div class="group relative p-4 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-[#8B5CF6]/40 transition-all duration-500 overflow-hidden hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(139,92,246,0.12)] flex flex-col h-[100px] justify-between">
                                ...
                                <!-- Decorative abstract wireframe based on index -->
    ...
    <footer class="py-8 border-t border-white/[0.05]">
        <div class="w-full px-8 sm:px-12 lg:px-16 xl:px-24 flex flex-col sm:flex-row items-center justify-between gap-6 text-[12px] text-white/30">
            <span>&copy; {{ date('Y') }} Sarham San. Hak cipta dilindungi.</span>
            
            <div class="flex items-center gap-5">
                <a href="#" aria-label="GitHub" class="hover:text-white transition-colors duration-300 hover:scale-110 transform">
                ...
                <a href="#" aria-label="LinkedIn" class="hover:text-blue-400 transition-colors duration-300 hover:scale-110 transform">
    ...
    {{-- ============================================ --}}
    <button id="scrollToTopBtn" class="scroll-top-btn" aria-label="Kembali ke atas">
    ...
    <script>
        ...
        setInterval(() => {
            roleText.style.transition = 'opacity 0.4s, transform 0.4s';
        ...
        // ===== JS Initialization (Preloader & Scroll Restoration) =====
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (sessionStorage.getItem('portfolioLoaded')) {
                ...
                if (bodyWrapper) {
                    ...
                    bodyWrapper.classList.add('overflow-x-hidden');
                }
            } else {
                ...
                setTimeout(() => {
                    if (preloader) {
                        ...
                        setTimeout(() => {
                            ...
                            if(bodyWrapper) bodyWrapper.classList.add('overflow-x-hidden');
                        }, 700);
                    }
                }, 4000); 
            }

            // 2. Scroll Memory Logic for Internal Links
            ...
            // Restore scroll immediately if returning from internal page
            const savedScrollPos = sessionStorage.getItem('portfolioScrollPos');
            if (savedScrollPos) {
                // Remove instantly to avoid affecting manual refresh later
                sessionStorage.removeItem('portfolioScrollPos');
                // Scroll instantly to saved position without smooth animation
                setTimeout(() => {
